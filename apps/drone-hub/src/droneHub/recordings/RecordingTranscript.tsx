...
export function RecordingTranscript({ recording, status }: { recording: DesktopRecording | null; status: DesktopRecording['status'] }) {
  ...
  const preview = status === 'recording';
  ...
  return <div className="max-h-[45vh] space-y-3 overflow-y-auto text-sm">
    ...
    {preview ? <p className="text-xs text-[var(--muted)]">{recording.liveTranscription ? 'Live preview updates about every 30 seconds. Speaker labels appear after stop.' : 'Live preview is off. The transcript will be created after stop.'}</p> : null}
