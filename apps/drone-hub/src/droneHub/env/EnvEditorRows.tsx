...
export function EnvEditorRows({
  ...
  return (
    <div className="flex flex-col gap-2">
      {entries.length === 0 ? (
        <div className="rounded border border-dashed border-[var(--border-subtle)] px-3 py-3 text-11 text-[var(--muted-dim)]">
          {emptyMessage}
        </div>
      ) : (
        entries.map((entry) => (
          <div key={entry.id} className={`${gridClassName} gap-2`}>
            <input
              ...
              disabled={disabled}
              className="h-9 rounded border border-[var(--border-subtle)] bg-[var(--panel-raised)] px-3 font-mono text-11 text-[var(--fg)] focus:outline-none"
              placeholder="KEY"
            />
            <input
              value={entry.value}
      ...
      <div>
        <button
          ...
          onClick={onAdd}
          disabled={disabled}
          className={`h-8 rounded border px-3 text-10 font-[var(--weight-semibold)] tracking-wide uppercase ${
            disabled
              ? 'cursor-not-allowed border-[var(--border-subtle)] text-[var(--muted-dim)]'
