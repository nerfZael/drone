    ...
    <div ref={commitLayoutRef as React.RefObject<HTMLDivElement>} className="flex-1 min-h-0 overflow-hidden flex">
      ...
      >
        <div className="sticky top-0 z-10 px-2.5 py-1.5 border-b border-[var(--border-subtle)] bg-[var(--panel-raised)]/95 backdrop-blur">
          <div className="text-9 font-[var(--weight-semibold)] tracking-wide uppercase text-[var(--muted-dim)]" style={{ fontFamily: 'var(--display)' }}>
            Commit List
          </div>
            ...
            return (
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <span className="font-mono text-10 text-[var(--accent)]">{shortSha(commit.sha)}</span>
                  <span className="text-9 text-[var(--muted-dim)]" title={when.title}>
                    {when.short}
                  </span>
                </div>
                <div className="mt-1 text-11 font-[var(--weight-semibold)] text-[var(--fg-secondary)] line-clamp-2">{commit.subject}</div>
                <div className="mt-1 text-10 text-[var(--muted)] truncate">{commit.authorName || 'Unknown author'}</div>
              </button>
            );
          })}
        </div>
      ...
      <div className="flex-1 min-w-0 min-h-0 overflow-hidden flex flex-col">
        {!selectedCommit ? (
          <div className="flex-1 min-h-0 overflow-auto px-3 py-3 text-11 text-[var(--muted)]">
            Select a commit to inspect its file-level changes.
          </div>
        ) : activeCommitDetailsError ? (
          <div className="flex-1 min-h-0 overflow-auto px-3 py-3 text-11 text-[var(--red)]">{activeCommitDetailsError}</div>
        ) : activeCommitDetailsLoading && !activeCommitDetails ? (
          <div className="flex-1 min-h-0 overflow-auto px-3 py-3 text-11 text-[var(--muted)]">Loading commit details…</div>
        ) : (
          <>
            <div className="px-2.5 py-2 border-b border-[var(--border-subtle)] bg-[var(--surface-soft)]">
              <div className="flex items-center gap-2 flex-wrap">
                <span className="font-mono text-11 text-[var(--accent)]">{shortSha(selectedCommit.sha)}</span>
                <span className="text-12 font-[var(--weight-semibold)] text-[var(--fg-secondary)]">{selectedCommit.subject}</span>
              </div>
              <div className="mt-1 flex items-center gap-1.5 flex-wrap text-10 text-[var(--muted)]">
            ...
            {commitEntries.length === 0 ? (
              <div className="flex-1 min-h-0 overflow-auto px-3 py-3 text-11 text-[var(--muted)]">This commit has no file diffs to display.</div>
            ) : viewMode === 'stacked' ? (
                  ...
                  return (
                    <section key={`commit:${selectedCommit.sha}:${entry.path}`} className="group/file rounded border border-[var(--border-subtle)] bg-[var(--surface-softest)] overflow-hidden">
                      <div className="px-2.5 py-1.5 border-b border-[var(--border-subtle)] bg-[var(--panel-raised)]/70 flex items-center gap-2">
                        <span
                          className={`inline-flex items-center justify-center min-w-[32px] h-5 rounded border text-10 font-mono ${badgeTone(entry)}`}
                ...
                <div className="flex-1 min-w-0 min-h-0 overflow-auto bg-[var(--chat-background)]">
                  <div className="sticky top-0 z-10 px-2.5 py-1.5 border-b border-[var(--border-subtle)] bg-[var(--panel-raised)]/95 backdrop-blur flex items-center justify-between gap-2">
                    ...
                    <div className="inline-flex items-center gap-1">
                      ...
                      <div className="text-9 text-[var(--muted-dim)] font-mono whitespace-nowrap">{shortSha(selectedCommit.sha)}</div>
                    </div>
                  </div>
                  {!selectedCommitFileEntry ? (
