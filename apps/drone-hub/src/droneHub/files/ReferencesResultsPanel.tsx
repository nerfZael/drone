  ...
  return (
    <div className="border-t border-[var(--border-subtle)] bg-[var(--surface-inset)]">
      <div className="h-9 px-3 flex items-center justify-between gap-3 border-b border-[var(--border-subtle)]">
        <div className="min-w-0 text-[var(--text-11)] font-[var(--weight-semibold)] text-[var(--fg-secondary)]">
        ...
        <button
          type="button"
          onClick={onClose}
          className="h-6 px-2 rounded border border-[var(--border-subtle)] text-[var(--text-10)] font-[var(--weight-semibold)] text-[var(--muted)] hover:bg-[var(--hover)] hover:text-[var(--fg-secondary)]"
        >
          Close
        </button>
      </div>
      {state.error ? (
        <div className="px-3 py-2 text-[var(--text-11)] text-[var(--red)] bg-[var(--red-subtle)]">
          {state.error}
        </div>
      ) : state.loading ? (
        <div className="px-3 py-3 text-[var(--text-11)] text-[var(--muted)]">Searching project...</div>
      ) : count === 0 ? (
        <div className="px-3 py-3 text-[var(--text-11)] text-[var(--muted)]">No references found.</div>
      ) : (
          ...
          {state.references.map((reference, index) => (
            ...
            >
              <div className="min-w-0 flex items-center gap-2">
                <span className="shrink-0 text-[var(--text-10)] font-mono text-[var(--accent)]">
                  {reference.line}:{reference.column}
                </span>
                <span className="min-w-0 truncate text-[var(--text-11)] font-medium text-[var(--fg-secondary)]">
                  {compactPathLabel(reference.path)}
                </span>
                <span className="shrink-0 text-[var(--text-10)] text-[var(--muted)]">
                  {languageLocationName(reference)}
                </span>
              </div>
              {reference.preview ? (
                <div className="mt-0.5 truncate pl-[46px] font-mono text-[var(--text-10)] text-[var(--muted)]">
                  {reference.preview}
                </div>
              ) : null}
            </button>
