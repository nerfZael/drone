  ...
  return (
    <div className="w-full h-full bg-[var(--panel-alt)] overflow-auto relative">
      ...
      <div className="px-3 py-2 text-[var(--text-11)]">
        {portsError && <div className="text-[var(--text-11)] text-[var(--red)] truncate" title={portsError}>{portsError}</div>}
            ...
            {portRows.map((p) => {
              ...
              return (
                <div key={`${p.containerPort}:${p.hostPort}`} className="flex items-center justify-between gap-3">
                  <span className="text-[var(--muted-dim)] flex-shrink-0 tabular-nums inline-flex items-center gap-1.5 font-mono text-[var(--text-10)]" title="container → host">
                    {isReachable ? (
                      <span
                  ...
                  <div className="min-w-0 flex items-center gap-1.5 font-mono text-[var(--text-10)]">
                    <a
                      href={routedUrl}
                      target="_blank"
                      rel="noreferrer"
        ...
        <div className="mt-2 pt-2 border-t border-[var(--border-subtle)] flex flex-col gap-1.5">
          <div className="flex items-center justify-between gap-3">
            <span className="text-[var(--text-10)] text-[var(--muted-dim)] tracking-wide uppercase" style={{ fontFamily: 'var(--display)' }}>Agent</span>
            <span className="min-w-0 truncate text-[var(--muted)] text-[var(--text-11)]" title={agentLabel}>
              {agentLabel}
            </span>
          </div>
          <div className="flex items-center justify-between gap-3">
            <span className="text-[var(--text-10)] text-[var(--muted-dim)] tracking-wide uppercase" style={{ fontFamily: 'var(--display)' }}>Chat</span>
            <span className="min-w-0 truncate text-[var(--muted)] text-[var(--text-11)] font-mono" title={chatName}>
              {chatName}
            </span>
          </div>
        </div>
