...
export function CollapsibleOutput({ text, ok }: { text: string; ok: boolean }) {
  ...
  return (
    <div className="relative">
      {!showAll && hidden > 0 && (
        <div className="mb-2 flex items-center justify-between gap-2">
          ...
          <button
            ...
            className="text-11 font-[var(--weight-semibold)] text-[var(--accent)] hover:text-[var(--fg)] transition-colors"
          >
      ...
      <pre
        className={`whitespace-pre-wrap text-12-5 leading-[1.6] font-mono ${
      ...
      {showAll && isLong && (
        <button
          ...
          className="mt-2 flex items-center gap-1 text-11 font-medium text-[var(--accent)] hover:text-[var(--fg)] transition-colors"
        >
