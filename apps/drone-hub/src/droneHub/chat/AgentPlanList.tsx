...
export function AgentPlanList({
  ...
  const showPlanItems = embedded || running || planExpanded;
  ...
  return (
    <section
      className={`${embedded ? '' : 'mt-2.5'} ${
        showTopDivider && !embedded ? 'border-t border-[var(--border-subtle)] pt-2.5' : ''
      } ${className}`}
      aria-label="Plan"
    >
      <div
        className={`${showPlanItems ? 'mb-2' : ''} flex min-h-7 items-center justify-between gap-3`}
      >
        <div className="min-w-0">
          {running || embedded ? (
            <div className="flex items-baseline gap-1.5">
              <span className="text-10 font-medium text-[var(--muted)]">Plan</span>
              <span className="font-mono text-9 tabular-nums text-[var(--muted-dim)]">
                ({progressLabel})
              </span>
            </div>
          ) : (
            <button
              ...
              className="flex items-center gap-1 text-10 font-medium text-[var(--muted)] transition-colors hover:text-[var(--accent)] focus-visible:text-[var(--accent)] focus-visible:outline-none"
        ...
        <>
            ...
            {visibleItems.map((item, index) => {
              ...
              return (
                <li
                  ...
                  className="grid grid-cols-[16px_minmax(0,1fr)] items-start gap-2 text-11 leading-[1.45]"
                >
                  <span
                    className="mt-[1px] flex h-4 w-4 items-center justify-center"
                    aria-hidden="true"
                  >
                    {done ? (
                      ...
                      <span
                        className={`h-2 w-2 rounded-full border ${cancelled ? 'border-[var(--muted-dim)] opacity-45' : 'border-[var(--accent-muted)]'}`}
                      />
                    )}
                  </span>
                  <span
          ...
          {hasHiddenItems ? (
            <button
              ...
              className="mt-2 text-10 font-medium text-[var(--muted)] transition-colors hover:text-[var(--accent)] focus-visible:text-[var(--accent)] focus-visible:outline-none"
              aria-expanded={stepsExpanded}
            >
              {stepsExpanded
                ? 'Show fewer steps'
                : `Show ${plan.items.length - DEFAULT_VISIBLE_ITEMS} more`}
            </button>
          ) : null}
        </>
      ) : null}
