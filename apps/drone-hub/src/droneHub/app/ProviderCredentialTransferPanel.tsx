...
export function ProviderCredentialTransferPanel({
  ...
  return (
    <section>
      <div className="text-10 font-[var(--weight-semibold)] uppercase tracking-wider text-[var(--accent)]">
      ...
      {selfIsAdministrator === false ? (
        <div className="mt-3 text-11 text-[var(--yellow)]">
          This device is not an administrator and cannot receive provider credentials.
        </div>
      ) : null}
      {sources.length === 0 ? (
        <div className="mt-3 text-11 text-[var(--muted)]">Pair another Hub first.</div>
      ) : (
        <div className="mt-4 flex flex-wrap items-end gap-2">
          <label className="grid min-w-56 flex-1 gap-1">
            ...
            <select
              ...
              disabled={busy !== null}
              className="h-9 rounded border border-[var(--border)] bg-[var(--panel)] px-3 text-12 text-[var(--fg)]"
            >
              {sources.map((device) => (
          ...
          <button
            ...
            className="h-9 rounded border border-[var(--accent-muted)] bg-[var(--accent-subtle)] px-3 text-11 font-[var(--weight-semibold)] text-[var(--fg)] disabled:opacity-50"
          >
      ...
      {message ? <div className="mt-3 text-11 text-[var(--green)]">{message}</div> : null}
      {error ? <div className="mt-3 text-11 text-[var(--red)]">{error}</div> : null}
    </section>
  );
}
