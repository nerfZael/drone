...
import { UiSegmentedControl, UiTabs } from '../../ui/components';
import {
  SKILL_FILE_KIND_OPTIONS,
  type SkillFileDraft,
  type SkillFileKind,
} from './skill-library-model';
import { SkillFilesWorkspace } from './SkillFilesWorkspace';
...
export function SkillLibrarySection({ skillLibrary }: { skillLibrary: UseSkillLibraryResult }) {
  ...
  const [skillsView, setSkillsView] = React.useState<'details' | 'files'>('details');
  const {
    skills,
    skillsSaving,
    skillsDeleting,
  ...
  const handleSkillsViewChange = React.useCallback(
    ...
    [packageDraftDirty, skillLibrary, skillsView],
  );
  ...
  return (
    <div className="flex min-w-0 flex-col gap-5">
      ...
      <UiTabs
        ...
        options={[
          { value: 'skills', label: 'Skills', badge: skills.length },
      ...
      {libraryTab === 'import' ? (
          ...
          actions={
            <>
              <UiSegmentedControl
                ...
                value={skillsView}
                onValueChange={handleSkillsViewChange}
                className="p-0.5 [&_button]:h-6 [&_button]:min-w-[3.75rem]"
                options={[
              ...
              <button
                ...
                disabled={skillsSaving || skillsDeleting}
                className={buttonClassName('secondary', skillsSaving || skillsDeleting)}
              ...
              <button
                type="button"
                onClick={() => void (skillsView === 'files' ? savePackageDraft() : saveDraft())}
                disabled={skillsSaving || skillsDeleting}
                className={buttonClassName('primary', skillsSaving || skillsDeleting)}
                style={{ fontFamily: 'var(--display)' }}
              >
        ...
        >
          {skillsView === 'files' ? (
            <SkillFilesWorkspace skillLibrary={skillLibrary} />
          ) : (
            <SettingsSplitView>
              ...
              <SettingsDetail className="flex flex-col gap-5">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button
                      ...
                      disabled={!draft.id || skillsDeleting || skillsSaving}
                      className={buttonClassName(
                        'danger',
                        !draft.id || skillsDeleting || skillsSaving,
                      )}
                      style={{ fontFamily: 'var(--display)' }}
                    >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                  ...
                  <label className="md:col-span-2 flex flex-col gap-1">
                    ...
                    <input
                      ...
                      className={inputClassName()}
                      placeholder="Short description used for skill discovery."
                    />
                  </label>
                  <label className="flex flex-col gap-1">
                  ...
                  <label className="flex flex-col gap-1">
                    ...
                    <input
                      ...
                      onChange={(e) => updateDraftField('compatibility', e.target.value)}
                      className={inputClassName()}
                      placeholder="codex,claude,cursor,opencode,pi"
                    />
                  </label>
                </div>
                ...
                <SettingsSection
                  compact
                  title="Package files"
                  description="SKILL.md is generated automatically."
                  actions={<span className="dh-type-menu-meta">{fileCountLabel}</span>}
                >
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <div className="flex flex-wrap items-center gap-2">
                  ...
                  <div className="flex flex-col gap-3">
                    {draft.files.length === 0 ? (
                      ...
                      draft.files.map((file: SkillFileDraft) => {
                        const option =
                          SKILL_FILE_KIND_OPTIONS.find((entry) => entry.value === file.kind) ??
                          SKILL_FILE_KIND_OPTIONS[3];
                        return (
                          <div
                            key={file.localId}
                            className="flex flex-col gap-3 border-t border-[var(--border-subtle)] pt-3 first:border-t-0 first:pt-0"
                          >
                            <div className="grid grid-cols-1 md:grid-cols-[minmax(0,1fr)_140px_auto] gap-2 items-end">
                              <label className="flex flex-col gap-1">
                                ...
                                <input
                                  value={file.path}
                                  onChange={(e) =>
                                    updateDraftFile(file.localId, { path: e.target.value })
                                  }
                                  className={`${inputClassName()} font-mono`}
                              ...
                              <label className="flex flex-col gap-1">
                                ...
                                <select
                                  value={file.kind}
                                  onChange={(e) =>
                                    updateDraftFile(file.localId, {
                                      kind: e.target.value as SkillFileKind,
                                    })
                                  }
                                  className={inputClassName()}
                                >
                            ...
                            <textarea
                              value={file.content}
                              onChange={(e) =>
                                updateDraftFile(file.localId, { content: e.target.value })
                              }
                ...
                <div className="grid grid-cols-1 gap-5 xl:grid-cols-3 xl:divide-x xl:divide-[var(--border-subtle)]">
                  ...
                  <div className="flex flex-col gap-3 xl:px-4">
                    ...
                    <div className="grid grid-cols-1 gap-2">
                      <input
                        value={draft.claudeArgumentHint}
                        onChange={(e) => updateDraftField('claudeArgumentHint', e.target.value)}
                        className={inputClassName()}
                        placeholder="Argument hint"
                      />
                      <input
                        value={draft.claudeAllowedTools}
                        onChange={(e) => updateDraftField('claudeAllowedTools', e.target.value)}
                        className={inputClassName()}
                        placeholder="Allowed tools (comma-separated)"
                      />
                      <input
                        value={draft.claudeModel}
                        onChange={(e) => updateDraftField('claudeModel', e.target.value)}
                        className={inputClassName()}
                        placeholder="Model"
                      />
                      <input
                        value={draft.claudeContext}
                        onChange={(e) => updateDraftField('claudeContext', e.target.value)}
                        className={inputClassName()}
                        placeholder="Context"
                      />
                      <input
                        value={draft.claudeAgent}
                        onChange={(e) => updateDraftField('claudeAgent', e.target.value)}
                        className={inputClassName()}
                        placeholder="Agent"
                      />
                      <label className="inline-flex items-center gap-2 text-[var(--text-11)] text-[var(--muted)]">
                        <input
                          ...
                          onChange={(e) =>
                            updateDraftField('claudeUserInvocable', e.target.checked)
                          }
                        />
                        User invocable
                      </label>
                      <label className="inline-flex items-center gap-2 text-[var(--text-11)] text-[var(--muted)]">
                        <input
                          ...
                          onChange={(e) =>
                            updateDraftField('claudeDisableModelInvocation', e.target.checked)
                          }
                        />
                  ...
                  <div className="flex flex-col gap-3 xl:pl-4">
                    ...
                    <label className="inline-flex items-center gap-2 text-[var(--text-11)] text-[var(--muted)]">
                      <input
                        ...
                        onChange={(e) =>
                          updateDraftField('cursorDisableModelInvocation', e.target.checked)
                        }
                      />
                      Disable model invocation
                    </label>
