    ...
    <div className="dh-settings-section">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button
          ...
          className={`h-8 px-3 rounded text-11 font-[var(--weight-semibold)] tracking-wide uppercase border transition-all ${
      ...
      <div className="grid grid-cols-1 2xl:grid-cols-2 gap-4">
        <div className="flex flex-col gap-3 min-w-0">
          <div className="text-10 uppercase tracking-[0.08em] text-[var(--muted-dim)] font-[var(--weight-semibold)]">Archived drones</div>
          {archivedDronesLoading && !archivedDrones ? (
            <div className="text-12 text-[var(--muted-dim)]">Loading archived drones…</div>
          ) : archivedRows.length === 0 ? (
            <div className="py-4 text-11 text-[var(--muted-dim)]">No archived drones.</div>
          ) : (
            <div className="overflow-x-auto rounded border border-[var(--border-subtle)]">
              <table className="w-full min-w-[620px] text-left">
                <thead className="bg-[var(--surface-softest)]">
                  <tr>
                    <th className="px-3 py-2 text-10 uppercase tracking-[0.08em] text-[var(--muted-dim)] font-[var(--weight-semibold)]">Drone</th>
                    <th className="px-3 py-2 text-10 uppercase tracking-[0.08em] text-[var(--muted-dim)] font-[var(--weight-semibold)]">Archived</th>
                    <th className="px-3 py-2 text-10 uppercase tracking-[0.08em] text-[var(--muted-dim)] font-[var(--weight-semibold)]">Deletes</th>
                    <th className="px-3 py-2 text-10 uppercase tracking-[0.08em] text-[var(--muted-dim)] font-[var(--weight-semibold)]">Actions</th>
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <tr key={row.id} className="border-t border-[var(--border-subtle)]">
                        <td className="px-3 py-2 align-top">
                          <div className="text-12 text-[var(--fg-secondary)]">{row.name}</div>
                          <div className="text-10 text-[var(--muted-dim)] font-mono mt-0.5">{row.id}</div>
                        </td>
                        <td className="px-3 py-2 align-top text-11 text-[var(--muted-dim)]">
                          {new Date(row.archivedAt).toLocaleString()}
                        </td>
                        <td className="px-3 py-2 align-top text-11 text-[var(--muted-dim)]">
                          {new Date(row.deleteAt).toLocaleString()}
                          <div className="text-10 mt-0.5">
        ...
        <div className="flex flex-col gap-3 min-w-0">
          <div className="text-10 uppercase tracking-[0.08em] text-[var(--muted-dim)] font-[var(--weight-semibold)]">Archived chats</div>
          {archivedChatsLoading && !archivedChats ? (
            <div className="text-12 text-[var(--muted-dim)]">Loading archived chats…</div>
          ) : archivedChatRows.length === 0 ? (
            <div className="py-4 text-11 text-[var(--muted-dim)]">No archived chats.</div>
          ) : (
            <div className="overflow-x-auto rounded border border-[var(--border-subtle)]">
              <table className="w-full min-w-[720px] text-left">
                <thead className="bg-[var(--surface-softest)]">
                  <tr>
                    <th className="px-3 py-2 text-10 uppercase tracking-[0.08em] text-[var(--muted-dim)] font-[var(--weight-semibold)]">Drone</th>
                    <th className="px-3 py-2 text-10 uppercase tracking-[0.08em] text-[var(--muted-dim)] font-[var(--weight-semibold)]">Chat</th>
                    <th className="px-3 py-2 text-10 uppercase tracking-[0.08em] text-[var(--muted-dim)] font-[var(--weight-semibold)]">Archived</th>
                    <th className="px-3 py-2 text-10 uppercase tracking-[0.08em] text-[var(--muted-dim)] font-[var(--weight-semibold)]">Deletes</th>
                    <th className="px-3 py-2 text-10 uppercase tracking-[0.08em] text-[var(--muted-dim)] font-[var(--weight-semibold)]">Actions</th>
                  </tr>
                  ...
                  {archivedChatRows.map((row) => {
                    ...
                    return (
                      <tr key={key} className="border-t border-[var(--border-subtle)]">
                        <td className="px-3 py-2 align-top">
                          <div className="text-12 text-[var(--fg-secondary)]">{row.droneName}</div>
                          <div className="text-10 text-[var(--muted-dim)] font-mono mt-0.5">{row.droneId}</div>
                        </td>
                        <td className="px-3 py-2 align-top text-12 text-[var(--fg-secondary)]">{row.chatName}</td>
                        <td className="px-3 py-2 align-top text-11 text-[var(--muted-dim)]">
                          {new Date(row.archivedAt).toLocaleString()}
                        </td>
                        <td className="px-3 py-2 align-top text-11 text-[var(--muted-dim)]">
