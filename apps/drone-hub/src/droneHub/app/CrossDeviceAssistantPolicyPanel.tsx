...
const fieldClass =
  'min-w-0 rounded border border-[var(--border)] bg-[var(--panel)] px-2.5 py-2 text-11 text-[var(--fg)] outline-none focus:border-[var(--accent-muted)]';
...
export function CrossDeviceAssistantPolicyPanel({
  ...
  if (loading)
    return (
      <section className="rounded-[var(--radius-large)] border border-[var(--border-subtle)] p-4 text-12 text-[var(--muted)]">
        Loading workspaces…
      </section>
    );
  ...
  return (
    <section className={mode === 'thread' ? 'min-h-full bg-[var(--panel-alt)]' : ''}>
      <div className="flex flex-wrap items-start justify-between gap-3 border-b border-[var(--border-subtle)] p-4">
        <div>
          <div className="text-10 font-[var(--weight-semibold)] uppercase tracking-[.16em] text-[var(--yellow)]">
        ...
        {mode === 'sharing' ? (
          <div className="flex gap-2">
            ...
            <button
              type="button"
              onClick={() => addRoot()}
              className="px-2 py-2 text-11 font-[var(--weight-semibold)] text-[var(--muted)] hover:text-[var(--fg)]"
            >
              Add path manually
            </button>
          </div>
          ...
          <button
            type="button"
            onClick={onClose}
            className="px-2 py-1 text-11 font-[var(--weight-semibold)] text-[var(--muted)] hover:text-[var(--fg)]"
          >
            Close
          </button>
        ) : null}
        ...
        {mode === 'sharing' ? (
          <>
              ...
              >
                <div className="grid gap-2 border-b border-[var(--border-subtle)] p-3 sm:grid-cols-[1fr_2fr_auto_auto]">
                  ...
                  <button
                    ...
                    className="rounded border border-[var(--border-subtle)] px-3 text-10 font-[var(--weight-semibold)] text-[var(--fg)] disabled:opacity-50"
                  >
                    Browse
                  </button>
                  <button
                    ...
                    className="rounded px-2 text-10 font-[var(--weight-semibold)] text-[var(--red)]"
                  >
                    Remove
                  </button>
                </div>
                    ...
                    return (
                      ...
                      >
                        <div className="min-w-36 flex-1">
                          <div className="text-12 font-[var(--weight-semibold)] text-[var(--fg)]">
                            {device.name}
                          </div>
                          <div className="mt-0.5 text-9 text-[var(--muted)]">
                        ...
                        {(['read', 'write', 'execute'] as const).map((permission) => (
                          <label
                            key={permission}
                            className="flex items-center gap-1.5 text-10 text-[var(--muted)]"
                          >
                            <input
                              type="checkbox"
            ...
            {policy.roots.length === 0 ? (
              <div className="rounded border border-dashed border-[var(--border)] px-4 py-8 text-center text-11 text-[var(--muted)]">
          ...
          <div className="grid gap-3 p-3">
            {availableDevices.map((device) => {
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-2 text-12 font-[var(--weight-semibold)] text-[var(--fg)]">
                    <span
                      className={`h-1.5 w-1.5 rounded-full ${connected ? 'bg-[var(--green)]' : 'bg-[var(--muted-dim)]'}`}
                    />
                    {device.name}
                    <span className="text-9 font-normal uppercase tracking-wide text-[var(--muted-dim)]">
                      {connected ? 'Connected' : 'Offline'}
                    </span>
                  </div>
                  {!connected ? (
                    <div className="mt-2 text-10 text-[var(--muted)]">
                      {selectedCount > 0
                    ...
                    <div className="mt-2 text-10 text-[var(--muted)]">Loading workspaces…</div>
                  ) : remote?.error ? (
                        ...
                        return (
                          ...
                          >
                            <div className="min-w-36 flex-1">
                              ...
                              {changed ? (
                                <div className="mt-0.5 text-9 text-[var(--yellow)]">
                                  changed
                                </div>
                              ) : null}
                            </div>
                            {(['read', 'write', 'execute'] as const).map((permission) => {
                              ...
                              return (
                                <label
                                  key={permission}
                                  className={`flex items-center gap-1.5 text-10 ${allowed ? 'text-[var(--muted)]' : 'text-[var(--muted-dim)] opacity-40'}`}
                                >
                                  <input
                                    type="checkbox"
                                    disabled={!allowed}
                        ...
                        .map((target) => (
                          ...
                          >
                            <div className="min-w-0 flex-1">
                              <div className="truncate text-11 font-[var(--weight-semibold)] text-[var(--fg)]">
                            ...
                            <button
                              ...
                              className="text-10 font-[var(--weight-semibold)] text-[var(--red)]"
                            >
                              Remove
                            </button>
                          </div>
            ...
            {availableDevices.length === 0 ? (
              <div className="py-8 text-center text-11 text-[var(--muted)]">
            ...
            {policy.homeTargets.some((target) => target.threadId === threadId) ? (
              <button
                ...
                className="justify-self-start text-10 font-[var(--weight-semibold)] text-[var(--red)]"
              >
          ...
          <div className="p-4 text-11 text-[var(--muted)]">
      ...
      <div className="flex items-center gap-3 border-t border-[var(--border-subtle)] p-4">
        <button
          ...
          onClick={() => void save()}
          className="rounded border border-[var(--accent-muted)] bg-[var(--accent-subtle)] px-4 py-2 text-11 font-[var(--weight-semibold)] text-[var(--fg)] disabled:opacity-50"
        >
        ...
        {dirty ? (
          <>
            ...
            <button
              ...
              className="text-10 font-[var(--weight-semibold)] text-[var(--muted)]"
            >
              Discard
            </button>
          </>
        ) : saved ? (
          <span className="text-11 text-[var(--green)]">Saved</span>
        ) : null}
        {error ? <span className="text-11 text-[var(--red)]">{error}</span> : null}
      </div>
    </section>
  );
}
