      ...
      <div className="grid grid-cols-1 xl:grid-cols-[minmax(0,320px)_minmax(0,1fr)] gap-4">
        <div className="dh-settings-section">
          <div className="flex flex-col gap-2">
            ...
            <div className="text-[20px] font-[var(--weight-semibold)] text-[var(--fg-strong)]" style={{ fontFamily: 'var(--display)' }}>
              {activeProfileName ? formatProfileDisplayName(activeProfileName) : 'Default'}
            </div>
            <div className="text-11 text-[var(--muted-dim)] leading-relaxed">
          ...
          <div className="dh-settings-subsection">
            <div className="text-10 font-[var(--weight-semibold)] uppercase tracking-[0.08em] text-[var(--muted-dim)]">Resolved paths</div>
            <div className="text-11 text-[var(--muted-dim)] break-all">Drone: {profileSettings?.droneDataDir ?? 'Loading…'}</div>
            <div className="text-11 text-[var(--muted-dim)] break-all">DVM: {profileSettings?.dvmDataDir ?? 'Loading…'}</div>
          </div>

          <div className="flex flex-col gap-2">
            <div className="text-10 font-[var(--weight-semibold)] uppercase tracking-[0.08em] text-[var(--muted-dim)]">Create profile</div>
            <input
              value={createProfileDraft}
              onChange={(e) => setCreateProfileDraft(e.target.value)}
              className="h-10 rounded border border-[var(--border-subtle)] bg-[var(--surface-inset)] px-3 text-13 text-[var(--fg)] placeholder:text-[var(--muted-dim)] focus:outline-none focus:border-[var(--accent-muted)] transition-colors"
            ...
            <button
              ...
              className={`h-10 px-3 rounded text-11 font-[var(--weight-semibold)] tracking-wide uppercase border transition-all ${
        ...
        <div className="dh-settings-section">
          <div className="flex items-center justify-between gap-3">
            <div>
              ...
              <div className="text-12 text-[var(--muted)] mt-1">Switch, rename, or remove isolated Hub workspaces.</div>
            </div>
            <div className="text-11 text-[var(--muted-dim)]">{profileList.length} total</div>
          </div>

          {profileSettingsLoading && !profileSettings ? (
            <div className="text-12 text-[var(--muted-dim)]">Loading profiles…</div>
          ) : profileList.length === 0 ? (
            <div className="text-12 text-[var(--muted-dim)]">No profiles found.</div>
          ) : (
                          ...
                          {editing ? (
                            <div className="flex flex-col gap-2">
                              <input
                                ...
                                className="h-10 rounded border border-[var(--border-subtle)] bg-[var(--surface-inset)] px-3 text-13 text-[var(--fg)] focus:outline-none focus:border-[var(--accent-muted)] transition-colors"
                                disabled={renaming}
                              />
                              <div className="flex items-center gap-2">
                                ...
                                <button
                                  ...
                                  disabled={renaming}
                                  className="h-8 px-3 rounded text-10 font-[var(--weight-semibold)] tracking-wide uppercase border transition-all bg-[var(--surface-softest)] border-[var(--border-subtle)] text-[var(--muted)] hover:bg-[var(--hover)] hover:text-[var(--fg-secondary)]"
                                  style={{ fontFamily: 'var(--display)' }}
                                >
                                  Cancel
                                </button>
                            ...
                            <>
                              <div className="flex flex-wrap items-center gap-2">
                                ...
                                {item.active && (
                                  <span className="rounded-full border border-[var(--green-border)] bg-[var(--green-subtle)] px-2 py-0.5 text-10 font-[var(--weight-semibold)] uppercase tracking-[0.08em] text-[var(--green)]">
                                    Active
                                  </span>
                                )}
                              </div>
                              <div className="text-11 text-[var(--muted-dim)] break-all mt-2">Drone: {item.droneDataDir}</div>
                              <div className="text-11 text-[var(--muted-dim)] break-all">DVM: {item.dvmDataDir}</div>
                            </>
                          )}
                        </div>
