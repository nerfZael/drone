...
function formInputClass(disabled: boolean): string {
  return `h-10 rounded border border-[var(--border-subtle)] bg-[var(--surface-inset)] px-3 text-13 text-[var(--fg)] placeholder:text-[var(--muted-dim)] focus:outline-none focus:border-[var(--accent-muted)] transition-colors ${disabled ? 'opacity-60 cursor-not-allowed' : ''}`;
}
  ...
  return (
    <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
      <div className="flex flex-col gap-2 xl:col-span-2">
        <div className="text-10 font-[var(--weight-semibold)] uppercase tracking-[0.08em] text-[var(--muted-dim)]">Label</div>
        <input
          value={draft.label}
      ...
      <div className="flex flex-col gap-2">
        <div className="text-10 font-[var(--weight-semibold)] uppercase tracking-[0.08em] text-[var(--muted-dim)]">Source type</div>
        <select
      ...
      <div className="flex flex-col gap-2">
        <div className="text-10 font-[var(--weight-semibold)] uppercase tracking-[0.08em] text-[var(--muted-dim)]">Target path</div>
        <input
      ...
      {draft.sourceType === 'host-path' ? (
        <div className="flex flex-col gap-2 xl:col-span-2">
          <div className="text-10 font-[var(--weight-semibold)] uppercase tracking-[0.08em] text-[var(--muted-dim)]">Source path</div>
          <input
        ...
        <div className="xl:col-span-2 rounded border border-[var(--border-subtle)] bg-[var(--surface-softest)] px-3 py-3">
          <div className="text-10 font-[var(--weight-semibold)] uppercase tracking-[0.08em] text-[var(--muted-dim)]">Hub-managed source</div>
          ...
          <label className="mt-3 flex items-start gap-2 text-12 text-[var(--muted)]">
            <input
              type="checkbox"
