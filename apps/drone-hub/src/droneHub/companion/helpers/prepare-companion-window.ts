...
export function prepareCompanionWindow(source: Document, target: Document): () => void {
  ...
  style.textContent = `
    ...
    /* While files are dragged over the window nothing is a window drag region, so the whole card can take the drop. */
    [data-companion-file-drag] [data-companion-drag-handle] { -webkit-app-region: no-drag; }
