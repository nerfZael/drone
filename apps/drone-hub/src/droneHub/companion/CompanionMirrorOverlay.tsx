...
import type { CompanionMirrorSession } from '@drone/assistant-chat';
...
function RemoteSession({ session }: { session: CompanionMirrorSession }) {
  ...
  const act = async (action: 'approve' | 'discard') => {
    ...
    try { await mirror.command(session, action); }
  ...
  return <aside aria-label={label} className="pointer-events-auto rounded-xl border border-[var(--border)] bg-[var(--panel)] text-[var(--fg)] shadow-[var(--shadow-dialog)]">
    ...
    <div className="space-y-3 p-3">
      ...
      {session.captions ? <details><summary className="cursor-pointer text-xs text-[var(--muted)]">Recent live conversation</summary><div className="mt-2 max-h-52 overflow-y-auto whitespace-pre-wrap text-xs">{session.captions}</div></details> : null}
      ...
      {session.lastExecution ? <details><summary className="cursor-pointer text-xs text-[var(--muted)]">Latest execution · {session.lastExecution.execution.ok ? 'Applied' : 'Failed'}</summary>
