...
export function UiTableContainer({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div
      className={cn('overflow-x-auto rounded-[var(--radius-large)] border border-[var(--border-subtle)]', className)}
      {...props}
    />
  );
}
...
export function UiTableRow({ className, ...props }: React.HTMLAttributes<HTMLTableRowElement>) {
  return <tr className={cn('transition-colors hover:bg-[var(--surface-softest)]', className)} {...props} />;
}

export function UiTableHeaderCell({ className, style, ...props }: React.ThHTMLAttributes<HTMLTableCellElement>) {
  return (
    <th
      className={cn('px-3 py-2 text-[length:var(--text-9)] font-[var(--weight-semibold)] uppercase tracking-[0.08em]', className)}
