/** Real browser capture + startup queue, using Chrome's synthetic microphone. No API calls. */
import { mkdtemp, rm } from 'node:fs/promises';
import { tmpdir } from 'node:os';
import { join } from 'node:path';
import { spawn } from 'node:child_process';

const temp = await mkdtemp(join(tmpdir(), 'live-pcm-smoke-'));
...
const bufferModule = new URL('../../../packages/assistant-chat/src/live-audio-buffer.ts', import.meta.url).pathname;
await Bun.write(entry, `
import { openBrowserLivePcmAudio } from ${JSON.stringify(audioModule)};
import { LiveAudioBuffer } from ${JSON.stringify(bufferModule)};
globalThis.runSmoke = async () => {
  const sent = []; const errors = []; let captured = 0; let capturedBytes = 0;
  const queue = new LiveAudioBuffer(async (audio) => { sent.push(audio); }, (error) => errors.push(error));
  const abort = new AbortController();
  const audio = await openBrowserLivePcmAudio({ signal: abort.signal, onAudio: (data) => { captured++; capturedBytes += atob(data).length; queue.append(data); }, onError: (error) => errors.push(error) }, () => {});
  const wait = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
  try {
    await wait(600);
    const before = { captured, sent: sent.length };
    queue.connect(); await wait(300);
    const after = { captured, sent: sent.length, capturedBytes, sentBytes: sent.reduce((sum, data) => sum + atob(data).length, 0) };
    const audible = sent.some((data) => [...atob(data)].some((byte) => byte.charCodeAt(0) !== 0));
    audio.play(sent[0]); await audio.resume(); await wait(100);
    queue.close(); const ended = sent.length; const stoppedCapture = captured; abort.abort();
    await audio.release(); await wait(200);
    return { before, after, audible, ended, final: sent.length, stoppedCapture, finalCapture: captured, errors };
  } finally { queue.close(); await audio.release(); }
};
`);
...
try {
  ...
  if (!value.before.captured || value.before.sent !== 0 || value.after.sentBytes !== value.after.capturedBytes || !value.audible || value.final !== value.ended || value.finalCapture !== value.stoppedCapture || value.errors.length) {
    throw new Error(JSON.stringify(value));
  }
  console.log('Browser PCM capture/buffer/playback smoke passed:', JSON.stringify(value));
} finally {
