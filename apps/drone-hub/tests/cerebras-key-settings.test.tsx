...
import { CerebrasKeySettings } from '../src/droneHub/app/CerebrasKeySettings';
...
test('saves and clears the Hub key without retaining the submitted secret in the form', async () => {
  ...
  const fetch = async (url: string, init?: RequestInit) => {
    ...
    if (url === '/api/settings/companion') return Response.json({ ok: true, settings: {}, credentials: { cerebras: method !== 'DELETE' } });
  ...
  try {
    await act(async () => { root.render(<QueryClientProvider client={client}><CerebrasKeySettings /></QueryClientProvider>); await flush(); });
    const input = element.querySelector('input')!;
    // React's input event handler accepts pasted credentials.
    const propsKey = Object.keys(input).find(key => key.startsWith('__reactProps'))!;
    await act(async () => { (input as any)[propsKey].onChange({ target: { value: 'test-cerebras-secret' } }); });
    await act(async () => { Array.from(element.querySelectorAll('button')).find(button => button.textContent === 'Save')!.click(); await flush(); });
    expect(requests).toContainEqual({ url: '/api/settings/cerebras', method: 'POST', body: { apiKey: 'test-cerebras-secret' } });
    expect(input.value).toBe('');
    expect(element.textContent).toContain('Saved Cerebras API key.');
    await act(async () => { Array.from(element.querySelectorAll('button')).find(button => button.textContent === 'Clear')!.click(); await flush(); });
    expect(requests.some(request => request.url === '/api/settings/cerebras' && request.method === 'DELETE')).toBe(true);
    expect(element.textContent).toContain('Cleared stored Cerebras API key.');
  } finally {
