...
test('moving Companion retains component state and DOM, follows theme, hides when idle, and docks on native close', async () => {
  ...
  for (const [name, value] of Object.entries({ window: source, document: source.document, HTMLElement: source.HTMLElement, MutationObserver: source.MutationObserver, IS_REACT_ACT_ENVIRONMENT: true })) {
  ...
  try {
    ...
    expect(controls.at(-1)).toBe('show');
    await act(async () => {
