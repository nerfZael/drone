...
describeSocketSuite('playbook api', () => {
  ...
  test('launches a playbook from a task button and renders task template values', async () => {
    const repoPath = path.join(tempRoot, 'repo-task-button-playbook');
    fs.mkdirSync(repoPath, { recursive: true });
    ...
    const created = await apiFetch('/api/playbooks', {
      ...
      body: JSON.stringify({
        label: 'Task reviewer',
        messages: [
          'Review this task: {{task.title}}',
          'Description: {{task.description}}',
          'Keep literal: {{task.notARealField}}',
        ],
        actions: [
          {
            label: 'Write fix',
            messages: [
              'Implement a fix for {{task.title}} in {{task.repoPath}}.',
              'Unknown template stays visible: {{task.missingActionField}}',
            ],
          },
        ],
      }),
    ...
    await apiFetch('/api/settings/task-playbook-buttons', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        taskPlaybookButtons: [
        ...
        lanes: [
          {
            ...
            cards: [
              {
                id: 'task-1',
                title: 'Race condition in webhook handler',
                description: 'Two deliveries can commit duplicate credits.',
                typeId: 'bug',
                repoPath,
    ...
    const launched = await apiFetch('/api/tasks/task-1/run-button/btn-review', {
      method: 'POST',
    });
    expect(launched.r.status).toBe(202);
    const droneId = String(launched.data?.droneId ?? '');
    expect(droneId).toBeTruthy();
    expect(launched.data?.taskId).toBe('task-1');
    expect(launched.data?.button).toEqual({ id: 'btn-review', label: 'Review task' });

    const reg = await loadRegistry();
    expect(reg.pending?.[droneId]?.startupQueuedPrompts?.map((item: any) => item.prompt)).toEqual([
      'Review this task: Race condition in webhook handler',
      'Description: Two deliveries can commit duplicate credits.',
      'Keep literal: {{task.notARealField}}',
    ]);
    expect(reg.pending?.[droneId]?.playbook).toMatchObject({
      ...
      actions: [
        {
          label: 'Write fix',
          messages: [
            `Implement a fix for Race condition in webhook handler in ${repoPath}.`,
            'Unknown template stays visible: {{task.missingActionField}}',
          ],
        },
      ],
