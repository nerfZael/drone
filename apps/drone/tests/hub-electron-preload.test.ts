...
function desktopBridge(invoke: (channel: string, mode: string) => Promise<unknown>) {
  let bridge: any;
  vm.runInNewContext(fs.readFileSync(path.join(import.meta.dir, '../desktop/hub-electron-preload.cjs'), 'utf8'), {
    require: (name: string) => {
      ...
      return { contextBridge: { exposeInMainWorld: (_name: string, value: unknown) => { bridge = value; } }, ipcRenderer: { invoke } };
    },
