...
describe('device mesh capability events', () => {
  test('accepts only signed allowlisted bounded events and rate-limits an authenticated peer', async () => {
    ...
    try {
      ...
      // Replay and invalid-signature attempts consume the peer budget before signature work,
      // leaving 118 delivered events within the 120-event ceiling.
      ...
      for (let index = 0; index < 466; index += 1) {
        await (target.router as any).onMessage(
          connection,
          Buffer.from(JSON.stringify({ type: 'capability.event', event: 'unknown' })),
        );
      }
