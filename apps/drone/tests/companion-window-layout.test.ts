...
test('layout tools are separate immediate browser calls and unavailable in mobile sessions', async () => {
  ...
  for (const name of ['get_chat_window_layout', 'arrange_chat_windows']) {
    const tool = tools.find((t: any) => t.name === name);
    expect(tool).toBeDefined();
    await tool.execute('call', {}, undefined);
  }
  expect(calls).toEqual(['get_chat_window_layout', 'arrange_chat_windows']);
  const mobile = await runtime.customTools({ ...context, windowLayoutTools: false }, []);
  expect(mobile.some((t: any) => ['get_chat_window_layout', 'arrange_chat_windows'].includes(t.name))).toBe(false);
});
