...
Mobile starts the same runtime over the paired-device mesh instead of opening the browser-only WebSocket. The mesh capability owns no history: it binds one run to the initiating phone, streams the same status/activity/reply events, relays phone-local composer and editor tools, and cancels on overlay close or device revocation. The phone uses its existing one-shot transcription setup; Companion model, prompt, and enabled tools remain canonical on the Hub.
