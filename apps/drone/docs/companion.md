...
On desktop, one press of the proposal shortcut applies the ready review card. A double press of its default Caps Lock binding toggles auto-approve, which is off by default. A manual Apply sends the validated per-operation execution result back to the same Companion conversation as a tool result and starts a continuation so Companion can respond to what actually happened. While auto-approve is enabled, a complete proposal executes inside the original proposal tool call; its actual result is returned before the agent finishes, avoiding an intermediate approval request and a separate acknowledgement turn. After every operation completes successfully, the proposal is automatically discarded, whether manually or automatically approved. Failed or partially applied proposals remain until explicitly discarded. Every manual and automatic execution remains available from the Companion header's execution-history button until the session closes, including partial failures and per-operation results.
...
Desktop and mobile Companion offer an **Auto-approve proposals** toggle. It defaults off and is saved per Hub in the canonical backend settings repository, shared by both clients. Closing Companion or restarting the app or Hub preserves the preference. Clients restore it when they reconnect to their Hub. Nonempty proposals with an execution context apply inside the patch tool call when the saved setting is enabled. A later failed or cancelled agent turn does not undo that application. Pending drafts can also auto-apply after a completed turn if the setting was enabled after the patch returned. Failed or partially applied proposals remain available for review and are not automatically retried.
