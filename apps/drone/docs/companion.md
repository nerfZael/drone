...
In the desktop app, open Companion’s options menu and turn on **Floating window**. Companion stays a compact, frameless panel, requested always on top. It starts 16 pixels from the bottom-right of the primary display’s work area (clear of the taskbar/dock), fits its visible content, and grows upward for replies and menus. Turning the switch off or using the OS close shortcut returns it to the main app, restoring the main window if minimized. The in-panel **Close Companion** button retains its existing session-closing behavior. When there is no visible Companion content, the floating window hides and returns for the next session.

The choice lasts until the main app reloads or closes; it is not a saved Hub setting. Drag the header grip to reposition it; later content growth keeps that chosen bottom-right corner fixed. It sizes automatically rather than exposing resize borders. Position is not persisted across detaching again. The floating window belongs to the main app’s lifetime, but remains usable while the main window is minimized. It has a transparent background outside the rounded panel and no title bar or taskbar entry. It does not request visibility across every virtual desktop or above full-screen apps.
