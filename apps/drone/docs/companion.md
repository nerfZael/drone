...
The Companion options menu has a volume slider (0–200%, default 100%) for Companion's own sound: the Live voice and the shortcut cue sounds (start, send, pause, resume, cancel, close, reset). It is applied on top of the system volume, is stored per browser profile in local storage (`drone-hub:companion-volume`), takes effect while the voice is speaking, and plays the send cue when an adjustment ends. Above 100% it amplifies, with a limiter so loud speech does not clip. The `speak` tool below is not covered; it keeps using the Speech settings volume.
