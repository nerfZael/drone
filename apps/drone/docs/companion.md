...
Provider, model, reasoning, voice preferences, auto-approval, and speech mute remain shared Hub settings. Number shortcuts are focus-local and leave text inputs, editors, shortcut capture, menus, and modal dialogs alone. The former default **1–4** Hub shortcuts are cleared on upgrade; explicitly assigning a number in Shortcut settings overrides its session shortcut.

## Floating desktop window

In the desktop app, open Companion’s options menu and turn on **Floating window**. Companion stays a compact, frameless panel, requested always on top. It starts 16 pixels from the bottom-right of the primary display’s work area (clear of the taskbar/dock) and fits its visible content. The bar (the header row with the status, your message and the buttons) is the fixed point: it always spans the full window width and never moves or resizes while a session runs. Replies, proposals, editors and menus stack away from it into whichever half of the screen has more room: upward while the bar sits in the lower half, downward once it is dragged into the upper half, where the reply appears under the bar and the proposal tabs hang from the bottom edge instead of the top. Turning the switch off or using the OS close shortcut returns it to the main app, restoring the main window if minimized. The in-panel **Close Companion** button retains its existing session-closing behavior. When there is no visible Companion content, the floating window hides and returns in the same place for the next session.

The choice lasts until the main app reloads or closes; it is not a saved Hub setting. Drag the bar to reposition it; later content growth keeps the bar exactly where it was dropped, and crossing the middle of the screen switches the growth direction. Content is capped at the room available on that side rather than pushing the bar. It sizes automatically rather than exposing resize borders. The bar position (and with it the growth direction) stays where you left it, including while the window is hidden between sessions, and is saved for the next app start. The floating window belongs to the main app’s lifetime, but remains usable while the main window is minimized. It has a transparent background outside the rounded panel and no title bar or taskbar entry. It does not request visibility across every virtual desktop or above full-screen apps.
...
This toggle is desktop-only. Electron documents [always-on-top as unsupported on native Wayland](https://www.electronjs.org/docs/latest/api/browser-window#winsetalwaysontopflag-level-relativelevel), where the window behaves as a normal floating window. Wayland also restricts programmatic positioning, sizing and focus, so the requested corner placement and automatic sizing cannot be guaranteed there. Transparency depends on the desktop compositor. Windows, macOS and X11 use Electron’s normal floating level; this does not promise visibility over system UI or other desktops.

Focused regression check (no app build or Hub server required): `node apps/drone-hub/scripts/smoke-companion-window.cjs` (prefix with `xvfb-run -a` for Linux CI). It checks real Electron window creation, toggling, native close, visibility, state retention, Monaco typing/undo and cross-window menu/dialog keyboard focus. It also renders the actual Companion overlay and checks its compact listening size, primary-display corner placement, transparent background, that the bar keeps its screen position and width while replies grow upward from a low bar or downward from a high one, and growth/shrink behavior for options. Its editor loads the installed Monaco package locally.

Manual acceptance: switch modes during a recording, live conversation, and running proposal; verify no restart or duplicate work. Edit an unsaved prompt/instructions draft, move the window, type and undo, then save. Open history/subscriptions/proposal details; drag the panel, change theme, minimize the main app, use the OS close shortcut, and reopen Companion. Check placement and stacking on the target OS, especially native Wayland and macOS full-screen/Spaces.
...
- Drafting multiple independently editable proposals for group, drone, chat, and message operations, with explicit review and approval before execution.
...
Search results should include a short snippet, repository, drone, chat, turn ID, role, timestamp, rank, and stable references that can be passed to `read_chat`. Use hard limits and pagination. Normalize only user prompts, visible assistant output, and visible errors from each active stored turn; do not index reasoning, tool arguments/results, attachments, or hidden metadata. Maintain a small, rebuildable SQLite FTS5 index beside the canonical transcript store and update CLI rows in the same transcript transaction. Native rows are refreshed from durable history before a search, using the same index, token matching, ranking, and pagination. Active-chat deletion or archiving must remove its rows. FTS5 is enabled in the current Hub SQLite runtime, so no fallback or vector store is needed.
...
`create_proposal` creates a session-owned JSON document with a unique `targetId`, path, and revision. `list_proposals` discovers proposal IDs, titles, revisions, and statuses. `read_proposal({ targetId })` reads a specific document plus supported operation shapes; omitting the ID reads the selected proposal for compatibility. `apply_proposal_patch` updates that document but never performs the proposed operations. The browser validates the complete document after every edit, including strict fields, unique operation IDs, limits, and references to drones created earlier in the same proposal. This keeps the provider tool schema small while still giving the review and execution boundary a typed contract.

The proposal supports creating, renaming, and deleting groups, drones, and chats; creating normal or draft drones/chats; cloning container drones; cloning chat history; copying chat configuration without history; and sending ASAP or queued messages. Drone creation can optionally override runtime, volume persistence, branch source/remote branch, and the initial chat's agent, provider, model, reasoning, permission mode, and approval policy. Chat creation supports the same chat-scoped overrides. Every omitted setting continues to use the saved creation or chat default. Operations execute top-to-bottom after explicit user approval and stop after the first failure. A later operation can target a newly created or cloned drone with `$<operation id>`. Repository omissions resolve against the active repository captured when the proposal is first created, so later navigation cannot silently retarget it. Any execution attempt, including a partial failure, is terminal only for that proposal. Other proposals remain editable and executable. Companion can create a correction containing only unfinished work and call `discard_proposal({ targetId, baseRevision })` to dismiss obsolete drafts or failures without user housekeeping. Discard preserves execution history and never undoes completed operations. A numbered proposal strip docked to the top edge of the desktop Companion window, and a selector inside the mobile overlay, let users review pending proposals independently; a draft appears there and in the review card only once it holds at least one operation. The first reviewable proposal opens on its own; after that, neither a newly filled draft nor an `execute_proposal` awaiting review replaces the proposal already open, and pressing the open proposal's number hides or shows its card. Message operations show their delivery as ASAP or Queued, matching the chat composer. Each proposal retains the repository/device and conversation captured at creation across selection changes.
...
On desktop, one press of the proposal shortcut applies the ready review card. A double press of its default Caps Lock binding toggles auto-approve, which is off by default. A manual Apply sends the validated per-operation execution result back to the same Companion conversation as a tool result and starts a continuation so Companion can respond to what actually happened. While auto-approve is enabled, a complete proposal executes only when Companion calls `execute_proposal({ targetId, baseRevision })`; its actual result is returned before the agent finishes, avoiding an intermediate approval request and a separate acknowledgement turn. After every operation completes successfully, the proposal leaves pending review and remains in execution history, whether manually or automatically approved. Failed or partially applied proposals remain until the user or agent discards them. They do not block new proposals. Every manual and automatic execution remains available from the Companion header's execution-history button until the session closes, including partial failures and per-operation results.

Pending proposals remain editable across conversation turns: follow-up requests patch the existing draft, and Apply returns the exact reviewed snapshot and its revision. Manual continuations use the normal cancellation-safe run lifecycle and current settings without reloading the session solely to insert the result. Results are bound to the originating conversation; an unavailable conversation produces an explicit delivery error instead of sending the result to a replacement conversation.

Desktop and mobile check the current saved auto-approval setting when `execute_proposal` arrives. The tool requires the exact proposal target and latest revision, rejecting stale, empty, executing, or already-executed proposals. With auto-approval off it returns `{ applied: false, status: "pending_review", revision, operationCount }` without executing; the existing review card remains available for user Apply. Successful mobile applications also clear the draft so Companion can create another proposal in the same turn. `execute_proposal` calls are exempt from the generic 20-second browser-tool deadline because applying may take longer; they wait for the actual result, Stop, or disconnect. Stop/disconnect ends the wait, not necessarily already-started operations, and does not automatically retry or roll back an application.

The proposal card is the single preview surface for pending operations and remains visible with auto-approval enabled so unfinished drafts and failed applications can be reviewed or discarded. It shows each structural change and its execution status without inserting speculative groups, drones, or chats into the live sidebar model. The sidebar continues to render confirmed and ordinary optimistic application state, and applied proposal changes appear there through the normal registry refresh.
...
These workspace tools execute directly and do not use proposals or approval prompts. Existing browser tools and Drone Hub proposals retain their behavior, including saved proposal auto-approval. The runtime reuses Blip's workspace tool schemas, selection tools, patch engine, transfer engine, and local/remote adapters, plus the built-in assistant's filesystem executor.
...
Desktop and mobile Companion offer an **Auto-approve proposals** toggle. It defaults off and is saved per Hub in the canonical backend settings repository, shared by both clients. Closing Companion or restarting the app or Hub preserves the preference. Clients restore it when they reconnect to their Hub. Nonempty proposals with an execution context apply inside `execute_proposal` when the saved setting is enabled. `read_proposal` and `apply_proposal_patch` only read and edit the draft. A later failed or cancelled agent turn does not undo that application. Completing a turn or enabling auto-approval never executes a pending draft. Companion must explicitly request execution, or the user must Apply it. Failed or partially applied proposals remain available for review and are not automatically retried.
...
While a reply exists, the header shows a button that folds the reply away so only the bar remains, or shows it again. The choice is a lasting preference (`drone-hub:companion-reply-collapsed` in local storage), not a per-reply state: collapsed stays collapsed for later replies, and expanded shows each reply once it arrives, with nothing extra while Companion is working. Errors are shown either way. In the floating window the reply text can be selected and copied; the bar and the margin around the reply still drag the window.

## Companion home

Companion has one workspace of its own, **Companion home** (`companion-home`), stored on disk under `companion/home/` in the Hub data directory. It needs no grant: Companion can always read and write it, and can never run commands in it. It is the default workspace target until the user picks another default in Workspaces, and it is a valid transfer source and destination beside the selected workspaces. Everything attached to an instruction (captures and pasted text) is saved in its `uploads/` folder; Companion may create any other files and folders it wants there, and they persist across conversations and Hub restarts.

**Home files** in the Companion options menu opens Companion home in the main window, with the Hub's file explorer on the left and the editor (tabs, save, rename, delete, upload, previews) on the right. The Hub serves the folder through the regular drone file routes under the id `companion-home`. The explorer sits on the right, as in the workspace layout. A file name or path in a Companion reply is a link into Companion home, like file references in agent chats: a file opens in the editor (at its line, if one is given) and is selected in the explorer; a folder is selected and expanded without opening anything. Paths may be relative to Companion home or absolute inside it; anything else is reported as not being in Companion home. From the floating window the link raises the main window. The window follows changes Companion makes while it is open, without polling. Opening it fetches a fingerprint of the folder (`GET /api/companion/home/revision`), which also arms a debounced recursive file watcher on the Hub; when something changes, the Hub sends a `companion_home_changed` UI action over the event stream every Hub UI already holds, and the window fetches the fingerprint once more. It also does so when the window regains focus, which covers a Hub restart and platforms that cannot watch a folder recursively. When the fingerprint has changed the explorer reloads its root and every expanded folder, and open tabs whose file changed on disk reload. Tabs with unsaved edits are left alone and show the usual changed-on-disk notice instead. Entries are not dimmed as git-ignored even when the Hub data directory sits inside an ignored folder of a repository. Companion home is also listed at the top of **Workspaces**, as always available, with an Open button. The window lives beside the Companion overlay, so closing or hiding Companion does not discard open editors; closing the window with unsaved changes asks first.

The Companion bar has a speaker button that mutes Companion's speech and nothing else. It is the Hub's own Speech **muted** setting, so the `speak` tool is told that speech is muted and General settings stays in step, and it also silences the Live voice. Cue sounds keep playing. The Companion options menu has a volume slider (0–200%, default 100%) for Companion's own sound: the Live voice and the shortcut cue sounds (start, send, pause, resume, cancel, close, reset). It is applied on top of the system volume, is stored per browser profile in local storage (`drone-hub:companion-volume`), takes effect while the voice is speaking, and plays the send cue when an adjustment ends. Above 100% it amplifies, with a limiter so loud speech does not clip. The `speak` tool below is not covered; it keeps using the Speech settings volume.

Both `send_message` and `create_drone` proposal operations accept `attachmentPaths`: up to 8 files of any type from Companion home (uploads, or files Companion wrote there such as a Markdown transcription), 6 MB each. They reach the agent as real chat attachments; for `create_drone` they go with the drone's first message. The paths are shown for review on the proposal, are read before anything is created, and a path outside Companion home fails the operation.

Drone chats take any file as an attachment, for CLI agents as well as the native one: the Hub copies it into the drone's workspace under `.drone-hub/attachments/` and tells the agent its path. Images and text are additionally presented as such; any other type is listed as a file to open. The existing limits apply: 8 attachments, 6 MB each, 20 MB together.

Attachments in drone chats open in the same viewer Companion uses: pending attachments in the composer, the chips on sent messages (an image at full size from the drone, a text attachment's content fetched from it), and inline images in messages. Images zoom and pan; text opens read-only in Monaco. The viewer opens in the window the click came from.

Companion can call `view_images` with 1–8 paths to look at images in Companion home in a single call: earlier screenshots, attachments that were listed by path instead of shown inline, or images it stored there. Paths are relative to Companion home or absolute inside it; PNG, JPEG, GIF and WebP up to 6 MB each; the workspace file tools cannot do this because they refuse binary files. It runs on the Hub, is enabled by default, and is added to existing settings that had `show_on_screen` enabled (settings schema 16).

Companion can call `set_clipboard` to replace the text on the user's clipboard, for a command, snippet, link or drafted message they are about to paste. It acts immediately without a proposal, accepts plain text up to 100,000 characters, returns `copied: true` with the character count, and never reads the clipboard. The desktop app writes through the native clipboard, which works while Companion floats over another application; a browser-only Hub uses the web clipboard API, which only succeeds while its tab is focused and otherwise reports `CLIPBOARD_UNAVAILABLE`. The tool is enabled by default and is added to existing settings that already had `show_on_screen` enabled (settings schema 15).

Companion can call `speak` to queue a brief spoken message in the open desktop Hub, independently of Live voice. Speech never talks over the user: while a voice recording is starting or running in the Hub (Companion's, a chat's voice message, or voice-to-clipboard), queued speech waits and speech that is already playing pauses; it continues, from just before where it stopped, once the recording is paused, sent or discarded. Live voice is a conversation and is not affected. It uses the existing Speech settings (enabled, voice, volume and mute) and requires a configured GROQ key. Each call accepts up to 200 characters and an optional voice override. The result reports `queued` or `muted`; it does not confirm that playback was heard. Companion should avoid repeating replies that Live voice will already speak.
