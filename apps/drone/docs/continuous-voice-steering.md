...
- 2026-08-08: Added one lease-based mobile microphone coordinator shared by single-shot and continuous capture, including ownership-aware UI blocking and asynchronous native shutdown. Continuous startup is now cancellable while audio-mode setup or Android foreground-service binding is pending, with matching JavaScript lifecycle tests and an Android native generation guard. Rebased the native recording patch onto Expo Audio 57.0.3, coalesced concurrent native bind callers without lock-held continuations, rejected late timed-out callbacks, and kept notification denial non-fatal.
