...
Prompt tests additionally cover migration from the boolean-only setting, partial writes that preserve the other field, validation, the desktop and mobile settings hooks, separate mobile prompt permissions, and composition of each new session from the latest editable prompt plus the required Hub contract.
