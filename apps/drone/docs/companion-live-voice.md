...
Voice and backend work are independent. Follow-ups are serialized through the current Companion runtime. Corrections can wait behind an active task; voice interruption does not guarantee that an in-flight mutation was cancelled or changed. New speech paired with a pending delegation suppresses an older spoken result, but does not undo operations. A repeated notification alone does not hide the answer. Use the explicit stop control for urgent cancellation.

This integration uses the desktop/browser WebRTC path. Mobile Companion retains its existing voice input implementation.
...
3. Ask the chosen backend to run a lookup and prepare a proposal while continuing to speak. Verify current tool activity remains visible, exact results render, and manual/automatic proposal execution keeps its existing behavior.
4. Switch UI workspaces during speech. Confirm tools stay bound to the displayed voice target. Change the backend setting and verify the next backend run uses it.
