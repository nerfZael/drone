...
export async function searchWeb(input: WebSearchInput, env: NodeJS.ProcessEnv = process.env): Promise<WebSearchResult> {
  ...
  if (!response.ok) {
    throw new Error(providerError(bodyText, `Exa search failed: ${response.status}`));
  }
...
export async function fetchContent(input: FetchContentInput, env: NodeJS.ProcessEnv = process.env): Promise<FetchContentResult> {
  ...
  if (!response.ok) {
    throw new Error(providerError(bodyText, `Exa contents fetch failed: ${response.status}`));
  }
...
function cleanHttpUrl(raw: unknown): string {
  ...
  try {
    ...
    throw new Error('fetch_content url must be a valid http or https URL');
  }
  if (url.protocol !== 'http:' && url.protocol !== 'https:') {
    throw new Error('fetch_content url must use http or https');
  }
  return url.toString();
}
