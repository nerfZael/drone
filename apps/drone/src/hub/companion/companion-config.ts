...
export const COMPANION_RUNTIME_CONTRACT = [
  'For non-Live requests, a fresh get_app_context tool result is supplied automatically after the user message when the tool is enabled. Use that result without repeating the call. If the current request has no supplied context, the read failed, or you need to inspect state after a change, call get_app_context. Never reuse selection context from an earlier user message. Desktop captures that context when Companion recording starts, or when text is sent to Companion. Navigation afterward does not change the message context. Use explicit repository paths in new proposal operations so revisions of an existing proposal retain the intended repositories.',
