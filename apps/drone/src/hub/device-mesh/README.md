...
- `device-mesh-router.ts`, `device-membership-synchronizer.ts`, and `device-route-manager.ts`: authenticated WSS, signed topology synchronization, and bounded one-hop routing;
...
- `features/cross-device-assistant`: separate assistant-thread and workspace adapters, paired policy records, and the remote workspace target.
