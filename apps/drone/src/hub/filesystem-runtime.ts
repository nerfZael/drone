...
export function createFilesystemRuntime(dependencies: FilesystemRuntimeDependencies) {
  ...
  function buildFsSearchScript(opts: {
    ...
    return [
      ...
      '  candidate="$1"',
      '  subsequence="$2"',
      '  while [ -n "$subsequence" ]; do',
      '    needle="${subsequence:0:1}"',
      '    subsequence="${subsequence:1}"',
      '    case "$candidate" in',
      '      *"$needle"*) candidate="${candidate#*"$needle"}" ;;',
      '      *) return 1 ;;',
      '    esac',
      '  done',
      '  return 0',
      ...
      '      if [ "$base" = "$token" ]; then token_score=0',
      '      elif case "$base" in "$token"*) true ;; *) false ;; esac; then token_score=100',
      '      elif case "$base" in *"$token"*) true ;; *) false ;; esac; then token_score=200',
      '      elif case "$lower" in *"$token"*) true ;; *) false ;; esac; then token_score=300',
      '      elif is_subsequence "$base" "$token"; then token_score=400',
      '      elif is_subsequence "$lower" "$token"; then token_score=600',
      ...
      '    path_length=${#lower}',
      '    [ "$path_length" -le 999 ] || path_length=999',
      '    score=$((score * 1000 + path_length))',
      '    printf "%09d\t%s\n" "$score" "$rel"',
      '  done < <(list_files)',
      '}',
      'count=0',
      'while IFS=$\'\t\' read -r _rank rel; do',
      `  ${joinFullPath}`,
      ...
      'done < <(rank_files | sort -t $\'\t\' -k1,1n -k2,2)',
    ].join('\n');
  }
