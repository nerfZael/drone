...
export const ASSISTANT_SYSTEM_PROMPT_DEFAULT = [
  ...
  'Use clone_chat to copy an existing chat history and configuration within its drone. Set sideChat:true for a temporary fork at the latest available checkpoint; sideChat and draft cannot both be true.',
...
const ASSISTANT_TOOL_SUMMARY_DEFINITIONS: AssistantToolSummary[] = [
  ...
  },
  {
    ...
    label: 'Clone chat',
    category: 'actions',
    description: 'Clone chat history and configuration, optionally into a temporary side chat.',
  },
  {
    name: 'rename_chat',
    label: 'Rename chat',
...
export const ASSISTANT_DEFAULT_ENABLED_TOOL_NAMES =
  ASSISTANT_PRE_MCP_OPT_IN_DEFAULT_ENABLED_TOOL_NAMES.filter(
    (name) => name === 'ask_questions' || !ASSISTANT_DRONE_HUB_MCP_TOOL_NAME_SET.has(name),
  );
