...
import { commandForPid, execFileStdout, isNgrokHttpCommand } from '../host/process-inspection';
...
export function isLegacyRemoteNgrokCommand(command: string, port: number | null): boolean {
  return isNgrokHttpCommand(command, port);
}
...
export async function cleanupLegacyRemoteHub(
  ...
  const readCommandForPid = dependencies.commandForPid ?? commandForPid;
  ...
  for (const candidate of candidates) {
    ...
    const command = await readCommandForPid(state.pid);
    if (!command) {
      preservedStateFiles.add(candidate.stateFile);
      warn(
        `Could not verify legacy ${candidate.kind} process ${state.pid}; its state file was preserved.`,
      );
      continue;
    }
    ...
    if (await stopPid(state.pid)) {
      ...
      preservedStateFiles.add(candidate.stateFile);
      warn(
        `Legacy ${candidate.kind} process ${state.pid} did not stop; its state file was preserved.`,
      );
    }
  }
