...
import { normalizeOpenAiSpeechRequest, synthesizeSpeechWithOpenAi } from '../openai-speech';
import {
...
const SPEECH_QUEUE_MAX_JOBS = 100;
...
export interface OperationalRouteDependencies {
  ...
  resolveOpenAiApiKeySettings?: ServiceFunction;
...
export function registerOperationalRoutes(
  ...
  const {
    ...
    resolveOpenAiApiKeySettings = async () => ({ apiKey: null }),
  ...
  const reserveSpeechQueueSlot = () => {
    if (speechJobsInQueue >= SPEECH_QUEUE_MAX_JOBS) return null;
  ...
  apiRouter.post('/api/audio/speech', async ({ readJson, json }) => {
    ...
    try {
      ...
      const savedVoice = parseSpeechVoice(speechSettings.voice);
      const speechModel = parseSpeechModel(speechSettings.model)
        ?? (savedVoice ? inferLegacySpeechModel(savedVoice) : DEFAULT_SPEECH_MODEL);
      const modelDetails = speechModelDetails(speechModel);
      const speechVoice = savedVoice && modelDetails.voices.includes(savedVoice)
        ? savedVoice
        : modelDetails.defaultVoice;
      const request = modelDetails.provider === 'openai'
        ? normalizeOpenAiSpeechRequest({ text: body?.text, voice: body?.voice ?? speechVoice, model: speechModel })
        : normalizeGroqSpeechRequest({ text: body?.text, voice: body?.voice ?? speechVoice, model: speechModel });
      ...
      const credential = modelDetails.provider === 'openai'
        ? await resolveOpenAiApiKeySettings()
        : await resolveGroqApiKeySettings();
      if (!credential.apiKey) {
        reservation.cancel();
        json(400, {
          ok: false,
          error: `${modelDetails.provider === 'openai' ? 'OpenAI' : 'GROQ'} API key is not configured. Add it in Drone Hub settings.`,
        });
        return;
      }
      ...
      reservation.run(async () => {
        ...
        try {
          ...
          const currentCredential = modelDetails.provider === 'openai'
            ? await resolveOpenAiApiKeySettings()
            : await resolveGroqApiKeySettings();
          if (!currentCredential.apiKey) throw new Error(`${modelDetails.provider === 'openai' ? 'OpenAI' : 'GROQ'} API key is no longer configured.`);

          timeout = setTimeout(() => controller.abort(), SPEECH_TIMEOUT_MS);
          timeout.unref?.();
          const audio = modelDetails.provider === 'openai'
            ? await synthesizeSpeechWithOpenAi({ apiKey: currentCredential.apiKey, request: request as ReturnType<typeof normalizeOpenAiSpeechRequest>, signal: controller.signal })
            : await synthesizeSpeechWithGroq({ apiKey: currentCredential.apiKey, request: request as ReturnType<typeof normalizeGroqSpeechRequest>, signal: controller.signal });
          ...
          const message =
            error instanceof Error && error.name === 'AbortError'
              ...
              : errorMessage(error);
          hubLog('error', 'Speech job failed', { jobId, error: message });
          emitAssistantUiAction(
            {
              type: 'speech_error',
              jobId,
