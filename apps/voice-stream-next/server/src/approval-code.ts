...
export type ApprovalCodeSettings = {
...
export class ApprovalCodeRecognizer {
  ...
  constructor(private opts: ApprovalCodeSettings = {}) {}

  get isCollecting(): boolean {
    return this.collecting;
  }

  configure(opts: ApprovalCodeSettings): void {
    this.opts = { ...opts };
    this.reset();
  }
...
export function approvalCodeFromText(text: string): string | null {
  const recognizer = new ApprovalCodeRecognizer();
  const update = recognizer.accept(text, 0);
  if (update.type === 'completed') return update.code;
  if (update.type === 'collecting' && update.partialCode.length >= 4) {
    return recognizer.flush(900).type === 'completed' ? update.partialCode : null;
  }
  return null;
}
