...
class LocalCuePlayer {
    ...
    private fun buildCuePcm(cue: LocalCue): CuePcm {
        val tones = when (cue) {
            ...
            LocalCue.WAKE_PENDING -> listOf(Tone(540.0, 45, 0.12))
        ...
        return CuePcm(tones.flatMap { it.samples().asIterable() }.toByteArray(), tones.sumOf { it.durationMs })
    }
    ...
    private fun Tone.samples(): ByteArray {
        ...
        for (index in 0 until sampleCount) {
            ...
            val value = if (frequencyHz <= 0.0) 0 else (sin(2.0 * PI * frequencyHz * index / SAMPLE_RATE_HZ) * Short.MAX_VALUE * gain * fade).toInt()
    ...
    private data class Tone(val frequencyHz: Double, val durationMs: Int, val gain: Double = 0.72)
