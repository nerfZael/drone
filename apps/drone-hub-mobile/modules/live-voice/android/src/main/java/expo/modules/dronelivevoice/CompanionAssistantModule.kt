...
class CompanionAssistantModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    AsyncFunction("requestRole") {
      ...
      val intent = if (manager.isRoleAvailable(RoleManager.ROLE_ASSISTANT) && !manager.isRoleHeld(RoleManager.ROLE_ASSISTANT))
        manager.createRequestRoleIntent(RoleManager.ROLE_ASSISTANT)
      else Intent(Settings.ACTION_VOICE_INPUT_SETTINGS)
      try { activity.startActivityForResult(intent, 8401) }
