...
Besides channel events, the runtime logs its own: tool calls (`tool_called`, `tool_done`), runs (`run_started`, `run_finished` with usage), workers (`limb_spawned`, `limb_started`, `limb_queued`, `limb_revived`, `task_continued`, `cancel_requested`, `task_done`, `work_summary`), routing (`steered`, `rerouted`, `group_started`, `group_extended`, `group_finished`), coordination (`claimed`, `released`, `write_refused`, `discovery`, `handoff`), renames (`limb_renamed`), code limbs (`watch_installed`, `watch_fired`, `watch_woke`, `program_started`, `program_woke`, `program_finished`), review (`review_queued`, `review_started`, `review_requeued`, `message_reviewed`), health warnings (`health`), usage outside the limbs (`usage`, for work summaries and senses), chat updates (`chat_message_updated`), timers (`timer_set`, `timer`), senses (`sensed`, `senses_dropped`), and the session (`session_started`, `session_paused`, `session_resumed`, `session_restored`).
