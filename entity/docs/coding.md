...
| Workspace channel | Events: commit, test and typecheck results, diff stats, conflict checks, user file edits. Effects: create attempt, run evidence, freeze, steer, discard, promote to PR (`confirm` for promote, push and destructive commands) | Partly built: blip's file tools and `bash` over the workspaces the user grants the session (Read, Write, Run each), with claims per workspace ([parallel-conversation.md](parallel-conversation.md#workspace)). No worktrees, commits or evidence events |
