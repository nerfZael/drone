...
- **Heartbeat.** An internal 50 ms tick rechecks level watches. The head gets a heartbeat wake every 3 minutes while other limbs are running, never while idle, so ticking costs almost nothing.
