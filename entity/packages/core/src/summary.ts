...
export interface Summarizer {
  summarize(input: { task: string; activity: string }, signal: AbortSignal): Promise<WorkSummary>;
}
...
export function describeToolArgs(name: string, args: Record<string, unknown>): string {
  ...
  const value = pick('path', 'command', 'keys', 'key', 'query', 'worker', 'id', 'name', 'task', 'text', 'note', 'result', 'reason');
