...
## Lock-screen startup cancellation follow-up

Reproduced `Startup cancelled. Tap Retry to start Companion.` on the connected Samsung SM-F966B using the system assistant key while locked. At 22:00 local time, logcat showed `GrantPermissionsActivity` opening during Live startup, pausing MainActivity and immediately returning. Microphone, Bluetooth, and notification permissions were all already granted. `prepareMobileLiveAudio` checked the microphone grant but unconditionally requested notification permission; Expo forwards that request to Android even when already granted.

Live preparation now checks Android's notification permission before requesting it, avoiding that unnecessary activity transition. The launch hook also waits for the first active state before treating background events as cancellation: native layout readiness alone does not prove a cold activity has reached foreground. Explicit dismissal and leaving foreground during pending startup still cancel the press.

Regression coverage checks that granted notification permission causes no request, missing permission still requests access, a cold/background launch can become active and start once, and leaving foreground during hydration still cancels. The targeted permission/launch tests pass (17 tests), as do provider, settings, screen, and Live lifecycle tests (25 tests, run in isolated processes where module mocks conflict). Mobile TypeScript validation passes.

The arm64 release build passed and was installed on the Samsung at approximately 22:05 local time. Without opening the regular app, a system assistant key invocation started a fresh React process, attached the desktop workspace, and logged `Remote Live session ready` at 22:05:13. A second locked invocation after ending the first session reached Live at 22:05:50. Android reported keyguard `showing=true, occluded=true` and active, unsilenced `VOICE_COMMUNICATION` capture for Drone Hub. The first launch no longer opened `GrantPermissionsActivity`. These checks exercise the system assistant key via adb, not the physical side-button gesture or a spoken round-trip. UIAutomator could not reach idle during the animated Live surface; its old dump was not used as post-fix evidence. Back ended the sessions and returned to keyguard.
