...
describe('device protocol', () => {
  ...
  test('advertises Companion run controls as explicit permissions', () => {
    expect(COMPANION_CAPABILITY.operations).toEqual(['run.start', 'run.cancel', 'tool.result', 'workspaces.list', 'workspaces.update', 'live.start', 'live.event', 'live.ping', 'live.close', 'live.settings.get', 'live.settings.update']);
