...
Android **Pairing** now automatically starts a two-minute, foreground-only discovery window: an mDNS/DNS-SD browser plus a pairing-only listener/advertisement on TCP 8792. Desktop **Network & pairing** advertises its HTTPS Hub address and browses for nearby phones while visible, with expiring local-admin leases to stop abandoned windows. Phones can select a nearby Hub without typing its address; a fresh signed HTTPS descriptor must match the advertised identity before requesting approval. Desktop **Find phones** additionally probes visible Tailscale IPv4 peers. Desktop-initiated phone offers still require matching verification codes and normal approval. No accounts or hosted discovery service are added. Bootstrap HTTP exposes public pairing metadata only and is separate from the HTTPS/Tailscale data plane. See [the Android test guide](apps/drone-hub-mobile/PHONE_DISCOVERY_TEST.md) for limits, native build requirements, and real-device acceptance. Native iOS discovery/listening is not implemented.
